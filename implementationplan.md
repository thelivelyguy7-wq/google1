...
| 1.2 | `README.md`: what the engine is, how to run it, what each output means, the synthetic-data caveat | A new reader can run the pipeline and read the outputs without asking | **Done** |
...
| 3.3 | App shell: navigation, permanent synthetic-data banner, label chips, light and dark themes, phone width | Opens from disk with no server | **Done** |
...
| 3.18 | Synthetic-data banner and "X of Y" denominators on every page | Test passes | **Done** |
...
| 5.12 | Evidence matrix gains a separate primary-findings column tagged `[OBS-PRIMARY]` | Synthetic and primary evidence never merged |
...
| Risk | Mitigation |
|---|---|
| The synthetic corpus does not resemble real behaviour | Every corpus finding is a hypothesis; Phase 5 is designed to be able to reject the target segment; a quick-success contrast group is recruited |
...
| The UI makes synthetic findings look like real research | Permanent banner, label chips, "X of Y" denominators, verdicts empty until fieldwork (3.18, 3.19) |
...
Choosing or designing a solution; estimating production impact without Google data; sentiment analysis as a discovery method; any claim that the synthetic corpus describes real Google Photos users.
