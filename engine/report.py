...
def build_report(precomputed=None) -> str:
    ...
    w("|---|---|---|")
    w(f"| **Retrieval-related** | **{s0['relevant']}** of {s0['total']} | Contains a retrieval object, a memory statement and a behaviour/outcome statement about finding an existing photo (IDs `SIM-0001…`) |")
    ...
    w(f"| Not retrieval-related | {s0['irrelevant']} | Single-sentence `SIM-N…` records on sharing, settings, storage, battery, backup, editing, printing, collage, face-grouping defect |")
